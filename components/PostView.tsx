...
function PostView({ id, post }: { id: string; post: PostWithExtras }) {
  ...
  return (
    <Dialog open={isPostModal} onOpenChange={(open) => !open && router.back}>
      <DialogContent className="flex gap-o flex-col md:flex-row items-start p-0 md:max-w-3xl lg:max-w-5xl xl:max-w-6xl h-full max-h-[500px] lg:max-h-[700px] xl:max-h-[800px]">
        <div className="flex flex-col justify-between md:h-full md:order-2 w-full max-w-md">
          ...
          <div className="px-2 hidden md:block mt-auto border-b p-2.5">
            ...
            <time className="text-[11px] uppercase text-zinc-500">
        ...
        <div
          className="relative overflow-hidden h-96 md:h-[500px] lg:h-[700px] xl:h-[800px]
        max-w-3xl w-full"
        >
          <Image
            src={post.fileUrl}
